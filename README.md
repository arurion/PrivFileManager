...
- **パーミッション変更(chmod)**: 所有者/グループ/その他ごとにread/write/executeを
  チェックボックスで変更可能
- **フォアグラウンドアプリ検知(任意・要手動有効化)**: AccessibilityServiceで
  「今表示されているアプリのパッケージ名」のみを検知し、そのアプリのデータへワンタップで
  移動できます。**画面の文字や操作内容は一切取得しません**(`canRetrieveWindowContent="false"`
  で技術的にも取得不能にしています)。デフォルトでは無効で、設定画面からOSのアクセシビリティ
  設定を開いて明示的に有効化する必要があります
